...
Hosting and infrastructure for a serving API, if one gets built, belongs to the IaC/cloud domain — see [`platform-archetypes.md`](../../iac-cloud-data-engineering/references/platform-archetypes.md). Contract and schema versioning for that API follows the same principles `quality-data-engineering` teaches for any data contract — see its `data-contracts-and-schema-compatibility.md`.
