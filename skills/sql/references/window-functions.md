...
`RANK` and `DENSE_RANK` are not interchangeable here — they answer different questions, and either can return more than 3 rows once ties are involved. `RANK() <= 3` gives you the top 3 **rank positions**: if two products tie for 3rd place, both get rank 3, so `RANK() <= 3` returns 1st, 2nd, and *both* rows tied for 3rd — 4 rows, not 3, because a tie exactly at the cutoff adds rows rather than dropping any. `DENSE_RANK() <= 3` gives you the top 3 **distinct sales figures** instead of the top 3 rank slots — with a tie earlier in the ordering (say, two products tied for 1st: ranks `1, 1, 3, 4` under `RANK` become dense ranks `1, 1, 2, 3`), the row `RANK` would call rank 4 (excluded from the top 3) becomes dense rank 3 (included), so `DENSE_RANK() <= 3` pulls in a row `RANK() <= 3` would have left out. Pick `DENSE_RANK` when the business question is "everyone in the top 3 distinct sales figures," `RANK` when it's "the top 3 rank slots, however many rows that takes," and `ROW_NUMBER` only when you want exactly 3 rows and don't care which of several ties gets arbitrarily dropped.
