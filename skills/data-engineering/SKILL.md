...
- Not for single-domain tasks — let that domain's own skill trigger directly (`python-data-engineering`, `sql-data-engineering`, `spark-data-engineering`, `modeling-data-engineering`, `pipelines-architecture-data-engineering`, `quality-data-engineering`, `project-structure-data-engineering`). Streaming and IaC/cloud are in scope for the suite but have no skill yet — cover those domains yourself rather than trying to load one.
