...
| A job can't authenticate; a pipeline outside the provider needs to reach a store inside it; deciding between a public and a private endpoint | Per-provider identity attached to compute and federation for compute outside it; bulk read as the interesting permission, at the engine layer and the storage layer both; network placement as an independent control, with the private path named per provider | [identity-and-network-access.md](references/identity-and-network-access.md) |
| "It's encrypted at rest" is offered as the answer; a credential is about to be stored somewhere; someone proposes to "restrict who can decrypt" | What default encryption does and does not buy; each provider's own term for a key you control; where key access is a genuine second gate and where revocation is only an incident-response lever; the order to work a secret down, ending at the state file | [encryption-and-secrets.md](references/encryption-and-secrets.md) |
...
| Reading "encrypted at rest" as protection against an over-permissioned reader | The documented threat model is compromise of the physical medium; decryption is transparent to anyone already authorized, and excess permission is filed as an access-control failure | Audit read grants separately from encryption settings, at the engine layer and the storage layer both; see [encryption-and-secrets.md](references/encryption-and-secrets.md) |
