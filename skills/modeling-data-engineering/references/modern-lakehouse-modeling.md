...
**Medallion is not dbt's vocabulary.** "Bronze/silver/gold" is a Databricks/lakehouse term. dbt Labs' own published guidance uses a different, real vocabulary — staging, intermediate, marts — and never uses "medallion" anywhere in its docs. See [dbt-project-architecture.md](../../pipelines-architecture-data-engineering/references/dbt-project-architecture.md)'s "The layering convention — and why it isn't 'medallion'" section for that vocabulary in full; don't restate it here, and never present the two as one and the same terminology. They're an analogy some teams draw between two separate ecosystems, not a shared standard.
