...
Everything beyond that — the staging/intermediate/marts layering convention, model naming conventions, the dependency graph between models (`ref()`/`source()`), project structure — is dbt as an orchestrator for the transformation layer, not a SQL-language concern; that content lives in [pipelines-architecture-data-engineering's dbt-project-architecture.md](../../pipelines-architecture-data-engineering/references/dbt-project-architecture.md), alongside Airflow, Dagster, and Prefect.
...
| Treating dbt project/model-layering concerns as a SQL topic | Conflates the transformation-orchestration layer with the query language | SQL-level idempotency and tests stay here; architecture questions go to [pipelines-architecture-data-engineering's dbt-project-architecture.md](../../pipelines-architecture-data-engineering/references/dbt-project-architecture.md) |
