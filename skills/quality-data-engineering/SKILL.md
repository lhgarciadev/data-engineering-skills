...
| Deciding what to check, and in what order | Schema → null/completeness → uniqueness → range/domain → referential integrity → freshness → volume (a teaching heuristic, not a dbt-published ranking) | [quality-dimensions-and-validation.md](references/quality-dimensions-and-validation.md) |
...
| Quarantining bad rows without silently dropping them | A boolean flag column + explicit separation (AWS Glue Data Quality, Databricks Lakeflow), or a separate audit relation (dbt `store_failures`) | [failure-response-policies.md](references/failure-response-policies.md) |
...
| Blocking bad data from reaching consumers | A data-domain circuit breaker; the fallback pattern is documented separately — link to it instead of restating it | [quality-culture-and-governance.md](references/quality-culture-and-governance.md) |
...
| Assuming "quarantine" means bad rows disappear from the model automatically | AWS Glue and Databricks need a flag column plus explicit filtering; dbt's `store_failures` only writes a separate audit relation | Plan for writing the flag + filter yourself (AWS Glue/Databricks), or filtering explicitly if the model itself needs bad rows excluded (dbt); see [failure-response-policies.md](references/failure-response-policies.md) |
...
| Citing a vendor's numbered "pillars" framework as a neutral standard | Monte Carlo's observability pillars and dlt's data-quality pillars are both marketing framings | Name the publisher and what they sell; see [data-observability-and-lineage.md](references/data-observability-and-lineage.md) |
...
| Describing a data "circuit breaker" as serving the last good snapshot | The data-domain sources describe blocking/missing data, not an automatic fallback | Treat blocking and the last-good-snapshot fallback as two separate patterns from two separate sources; cross-link to `serving-pipeline-output.md` for the fallback instead of restating it; see [quality-culture-and-governance.md](references/quality-culture-and-governance.md) |
