...
2. **What the sources actually say about batch and streaming**, per the Dataflow verification. Do **not** write "batch is a special case of streaming" — that verification returned **CORREGIDO** on exactly that claim: the phrase appears verbatim in neither the VLDB paper nor Akidau's blog post. Use the paper's own framing instead ("the distinction of streaming or batch is largely irrelevant" from the model's perspective; engine choice reduced to correctness, latency, and cost), and if a directional statement is wanted, Akidau's blog-post line that "well-designed streaming systems actually provide a strict superset of batch functionality" — attributed to the blog post, not the paper. Note that the superset framing and the "special case" paraphrase assert the *same* relationship; what is wrong with the paraphrase is its attribution and wording, not its direction. The point to carry is why the same primitives — windows, aggregations — apply to both, which is stronger than "two separate technologies" and doesn't require ranking one as a subset of the other.
