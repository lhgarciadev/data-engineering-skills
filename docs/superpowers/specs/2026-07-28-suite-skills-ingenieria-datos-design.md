...
**Estado (2026-08-02):** el roadmap se amplió de 8 a **9** skills de dominio — `project-structure-data-engineering` (estructura de paquete, empaquetado y contratos de datos) se entregó el 2026-08-02 fuera del roadmap original y se incorporó formalmente a él ese mismo día, una vez confirmado el caso de uso real que motivó construirla (ver su propia spec, `docs/superpowers/specs/2026-08-02-project-structure-skill-design.md`, y la nota histórica en §8 sobre por qué no debe confundirse con la propuesta de "APIs" descartada antes). **7 de 9** skills de dominio entregadas: `python-data-engineering`, `sql-data-engineering`, `spark-data-engineering`, `pipelines-architecture-data-engineering`, `project-structure-data-engineering`, `quality-data-engineering`, `modeling-data-engineering` (entregada 2026-08-04, con énfasis especial en Data Vault 2.0 sobre los dos libros canónicos locales — ver `docs/superpowers/specs/2026-08-04-modeling-skill-design.md`), más la orquestadora `data-engineering`. Quedan 2 por construir: `streaming-data-engineering`, `iac-cloud-data-engineering` (ver sección 7, ítem 6, actualizado).
...
6. Por cada una de las 2 skills de dominio restantes (`streaming-data-engineering`, `iac-cloud-data-engineering` — `sql-data-engineering` y `spark-data-engineering` ya se entregaron 2026-07-29, `pipelines-architecture-data-engineering` se entregó 2026-07-30, `quality-data-engineering` se entregó 2026-08-03, y `modeling-data-engineering` se entregó 2026-08-04 con énfasis especial en Data Vault 2.0 sobre fuentes primarias locales, ver `docs/superpowers/specs/2026-07-28-sql-skill-design.md`, `docs/superpowers/specs/2026-07-29-spark-skill-design.md`, `docs/superpowers/specs/2026-07-30-pipelines-architecture-skill-design.md`, `docs/superpowers/specs/2026-08-03-quality-skill-design.md` y `docs/superpowers/specs/2026-08-04-modeling-skill-design.md`): el usuario aporta tópicos → investigación (web y/o fuentes primarias locales cuando existan) → redacción → validación de discoverability liviana (mismo proceso ya usado en las 7 skills entregadas).
...
- Recorte y cross-linking de contenido que hoy vive en `python-data-engineering` pero en rigor pertenece a otra skill futura: las menciones a Airflow/Dagster/Prefect en `skills/python-data-engineering/references/production-patterns.md` (territorio de `pipelines-architecture-data-engineering`) y a Great Expectations en `skills/python-data-engineering/references/data-validation.md` (territorio de `quality-data-engineering`). Ambas se aplican recién cuando esas skills existan — hacerlo antes dejaría referencias rotas. El cross-link hacia `spark-data-engineering` (desde `concurrency-and-the-gil.md`) ya se cerró (2026-07-29), el de Great Expectations hacia `quality-data-engineering` también (2026-08-03, parte de la entrega de esa skill — ver Estado arriba), y el de Airflow/Dagster/Prefect hacia `pipelines-architecture-data-engineering` también (2026-08-04): `production-patterns.md` (secciones "Idempotency and safe reruns") ahora enlaza a `pipelines-architecture-data-engineering/references/idempotency-and-backfills.md` para la contraparte de diseño del DAG, y ese archivo, que ya mencionaba a `python-data-engineering` en prosa, quedó con el hyperlink recíproco hacia `production-patterns.md`. Sin pendientes de cross-linking abiertos en este ítem.
