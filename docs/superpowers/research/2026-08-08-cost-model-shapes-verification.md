...
- **La separación de cargos: SUPPORTED.** La claim, según el brief, dice que el almacenamiento de objetos *"charges separately for storage, requests, and egress"*, y eso se verifica en los tres proveedores (S3, GCS y Azure Blob/ADLS): los tres cobran esos conceptos por separado. **Advertencia de encuadre para la skill —observación de esta verificación, no defecto de la claim—:** enumerarlos puede leerse como si fueran los tres componentes del precio, y ningún proveedor usa una taxonomía de tres. S3 enumera **seis** componentes de costo; GCS enumera **tres**, pero con nombres distintos y con las peticiones metidas dentro de otro componente ("Data processing"); Azure enumera una lista larga de *meters*, y su *meter* de "Data transfer" en Blob cubre solo copia entre regiones, no el egreso a internet. La formulación segura es "**almacenamiento, peticiones y transferencia son cargos separados, entre otros**", no "el costo se descompone en exactamente tres".
...
| 4 | Storage cobra por separado almacenamiento, peticiones y egreso; el egreso produce lock-in | **CORRECTED** por su segunda mitad. La separación de cargos es **SUPPORTED** y está verificada en S3/GCS/Azure; **la parte de "lock-in" es inferencia del repo, no afirmación de ningún proveedor**. Advertencia de encuadre añadida por esta verificación: ningún proveedor usa una taxonomía de tres (S3 enumera seis componentes, GCS mete las operaciones dentro de "Data processing", Azure usa una lista de meters y su meter "Data transfer" ni siquiera es el egreso a internet) |
